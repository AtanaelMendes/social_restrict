...
void initializeNotifications() async {
  WidgetsFlutterBinding.ensureInitialized();
   //sawait FirebaseMessaging.instance.requestPermission();
...
void getAndroidUsageStats() async {
  try {
    ...
    for (var info in infoList) {
      debugPrint('Funcao do FLUTTER tempo uso ${info.packageName} ${info.usage.inMinutes}');
    }
...
const AndroidNotificationChannel notificationChannel = AndroidNotificationChannel("coding is life foreground", "coding is life foreground service",
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: customerId == 0 ? const BackgroundMainPage(title: "Social Restrict") : const HomePage(),
    );
  }
}
